...
export function DataTab() {
  ...
  // Cleanup duplicate projects handler
  const handleCleanupProjects = async () => {
    if (!user) {
      toast.error('Please sign in to clean up projects');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `✅ Cleanup complete! ${result.results?.merge?.projectsArchived || 0} duplicate projects merged, ${result.results?.merge?.screensMoved || 0} screens consolidated.`,
        );
        // Reload the page to refresh project list
      ...
      toast.error('Failed to clean up projects. Please try again.');
    } finally {
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Cleanup Projects Confirmation Dialog */}
      <ConfirmationDialog
        isOpen={showCleanupConfirm}
        onClose={() => setShowCleanupConfirm(false)}
        title="Clean Up Duplicate Projects?"
        description="This will merge all duplicate 'My Designs' projects into one, consolidate screens, and recalculate screen counts. This action cannot be undone."
        confirmLabel="Clean Up"
        cancelLabel="Cancel"
        variant="primary"
        isLoading={isCleaningUp}
        onConfirm={handleCleanupProjects}
      />
              ...
              <CardHeader>
                <div className="flex items-center mb-2">
                  <motion.div
                    ...
                    whileTap={{ scale: 0.9 }}
                  >
                    <div className="i-ph-broom-duotone w-5 h-5" />
                  </motion.div>
                  <CardTitle className="text-lg group-hover:text-bolt-elements-item-contentAccent transition-colors">
                    Clean Up Projects
                  </CardTitle>
                </div>
                <CardDescription>Fix duplicate "My Designs" projects and recalculate screen counts.</CardDescription>
              </CardHeader>
              <CardFooter>
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-full">
                  <Button
                    ...
                    className={classNames(
                      'hover:text-bolt-elements-item-contentAccent hover:border-bolt-elements-item-backgroundAccent hover:bg-bolt-elements-item-backgroundAccent transition-colors w-full justify-center',
                      isCleaningUp || !user ? 'cursor-not-allowed' : '',
                    )}
                  >
                    {isCleaningUp ? (
                      <>
