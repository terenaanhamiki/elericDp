...
export const Preview = memo(({ setSelectedElement }: PreviewProps) => {
  ...
  useEffect(() => {
    ...
    // Debounce URL updates to prevent rapid flickering during AI building
    // Wait 500ms of inactivity before actually updating the iframe
    urlUpdateTimeoutRef.current = setTimeout(() => {
      const urlToSet = pendingUrlRef.current;

      if (urlToSet && urlToSet !== iframeUrl) {
        // Clear any existing loading timeout
        if (loadingTimeoutRef.current) {
          clearTimeout(loadingTimeoutRef.current);
        }

        // Slightly reduce opacity during update
        setIframeOpacity(0.97);

        // Update the URL
        setIframeUrl(urlToSet);
        setDisplayPath('/');
