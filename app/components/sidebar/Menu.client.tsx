...
export const Menu = () => {
  ...
  const loadEntries = useCallback(async () => {
    try {
      // Load from Supabase (cloud-synced projects)
      const projects = await supabasePersistence.getProjects();

      const projectItems: ChatHistoryItem[] = projects.map((project) => ({
        id: project.id,
        urlId: project.id,
      ...
      // Also load from IndexedDB for backward compatibility
      if (db) {
        const localChats = await getAll(db);
        const filteredLocal = localChats.filter((item) => item.urlId && item.description);

        // Merge: prioritize Supabase projects, add local-only chats
        const supabaseIds = new Set(projectItems.map((p) => p.id));
        const localOnly = filteredLocal.filter((local) => !supabaseIds.has(local.id));

        setList([...projectItems, ...localOnly]);
      } else {
  ...
  // TEMPORARILY DISABLED: Auto-open/close sidebar on mouse hover
  // useEffect(() => {
      ...
      >
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            <div className="flex gap-2">
              <a
                href="/"
                onClick={() => {
                  // Clear current project state to start fresh
                  clearCurrentProject();
          ...
          <div className="flex items-center justify-between text-sm px-4 py-2">
            <div className="font-medium text-gray-600 dark:text-gray-400">Your Projects</div>
            {selectionMode && (
