...
export class StreamingMessageParser {
        ...
        if (state.insideAction) {
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              // Skip non-file actions in design mode
              if (parsedAction === null) {
                // Find the closing tag and skip the entire action
                const actionCloseIndex = input.indexOf(ARTIFACT_ACTION_TAG_CLOSE, actionEndIndex);
  ...
  #parseActionTag(input: string, actionOpenIndex: number, actionEndIndex: number) {
    ...
    // DESIGN TOOL MODE: Only allow file actions, skip all others
    if (actionType !== 'file') {
      logger.debug(`Skipping unsupported action type '${actionType}' in design mode`);
      return null; // Return null to indicate this action should be skipped
    }
